...
export async function registerRoutes(app: Express): Promise<Server> {
  try {
    ...
    console.log('🔧 Setting up multer error handling...');
    ...
    console.log('✅ Multer error handling configured');
    ...
    console.log('🔧 Setting up Clerk webhooks...');
    setupClerkWebhooks(app);
    console.log('✅ Clerk webhooks configured');
      ...
      try {
        ...
        if (!user) {
          ...
          try {
            ...
            console.error(
              'Failed to create user for cellar save:',
              createError
            );
            return res.status(500).json({ message: 'Failed to create user' });
          }
    ...
    if (process.env.NODE_ENV === 'development') {
      app.get('/api/test-error-handler', (req, res, next) => {
        console.log(
          'Test error endpoint called - triggering intentional error'
        );
    ...
    // Database status check endpoint (development only)
    if (process.env.NODE_ENV === 'development') {
