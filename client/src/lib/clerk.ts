...
if (!isClerkConfigured) {
  if (!clerkPubKey) {
    console.warn('⚠️ Authentication disabled: VITE_CLERK_PUBLISHABLE_KEY not found in environment');
  } else if (!isDomainAllowed) {
    console.warn('⚠️ Authentication disabled: Domain not allowed for Clerk authentication');
  } else {
    console.log('🔒 Authentication configured but not working. Check Clerk dashboard settings.');
  }
} else {
