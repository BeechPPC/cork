    ...
    <Card className="bg-white border border-gray-200 hover:shadow-lg transition-shadow">
      <CardContent className="p-6">
        ...
        <div className="mb-4">
          ...
          <h3 className="font-poppins font-semibold text-lg text-slate mb-1 line-clamp-2">
            {wine.name}
          </h3>
          ...
          {wine.matchReason && (
            <p className="text-xs text-grape mb-3 italic">
              "{wine.matchReason}"
            </p>
          )}
          
          <div className="flex items-center space-x-2 text-sm mb-2">
            <span className="font-medium text-slate">{wine.priceRange}</span>
            <span className="text-gray-400">•</span>
            <span className="text-gray-600">{wine.abv}</span>
          </div>
          ...
          <div className="flex items-center space-x-2">
            {showSaveButton && (
              <Button 
                onClick={onSave}
                disabled={isLoading}
                className="flex items-center space-x-2 bg-grape text-white px-4 py-2 rounded-lg hover:bg-purple-800 transition-colors"
              >
                {isLoading ? (
